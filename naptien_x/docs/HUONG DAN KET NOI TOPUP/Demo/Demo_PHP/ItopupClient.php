...
class ItopupClient {
	...
	function directTopupMobile($requestID, $targetAccount, $amount){
		...
		$param = array(
			...
			if ($this->errorCode == 0){
				...
				echo "<h3>Balance: <font color=\"red\">".$merchantBalance."</font> VND</h3>";
			}
		}
	}
	...
	function partnerCheckProductDetail($productID, $quantity){
		echo "__________________________________________________<br>";
		$this->errorCode = -100;
		$this->errorMessage = "Not conncect!";
		if ($this->token == null || $this->token == "") {
		...
		if ($this->token == null || $this->token == ""){
			...
			$this->errorMessage = "Not conncect!";
			 ...
			 //Build list Items
			$product = array('productId'=> $productID, 'quantity' => $quantity);
			$checkProducts = array($product);
			$param = array(
									'username' => $this->username, 
									'token' => $this->token,
									'checkProducts' => $checkProducts);
			$result = $this->client->call('partnerCheckProductDetail', $param);				
			print_r($result);
			if ($result != null){
				echo "<h3>ErrorCode: <font color=\"red\">". $result[0]["errorCode"]."</font></h3>";
				echo "<h3>Message: <font color=\"red\">". $result[0]["errorMessage"]."</font></h3>";
			}
